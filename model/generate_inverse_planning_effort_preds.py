"""
Generate inverse-planning predictions for the effort experiment (inv_plan_effort).

Uses frozen actor parameters from forw_plan_effort + fitted alpha_observer from
inverse_planning_effort_fit_results.csv. For each variant produces a 4D
posterior over intimacy and unrolls to long-format CSVs:

  inv_plan_effort_preds_full.csv     — one row per (scenario, action, effort, intimacy_level)
  inv_plan_effort_preds_summary.csv  — expected_intimacy collapsed across the IntimacyLevels grid
"""
...
from fit_inverse_planning_effort import (
...
from model_utils import IntimacyLevels, SCENARIO_LABELS
...
def main():
    print("=" * 60)
    print("Generating inv_plan_effort predictions")
    print("=" * 60)
    ...
    print("\nLoading fitted alpha_observer values from inverse_planning_effort_fit_results.csv...")
    alpha_obs = load_fitted_alpha_observer(
        get_project_root() / "model" / "outputs" / "inverse_planning_effort_fit_results.csv"
    )
    ...
    full_path = output_dir / "inv_plan_effort_preds_full.csv"
    summary_path = output_dir / "inv_plan_effort_preds_summary.csv"
