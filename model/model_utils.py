...
# ==============================================================================
# Loaded from model/outputs/lm_scenario_params.csv when available. The access
# family of `_llm` model variants index into these tables with scenario_idx.
# If the CSV is missing, LLM_TABLES stays None and the `_llm` variants are
# unusable until `uv run python model/lm_scenario_params.py` is run.


def load_lm_scenario_params(filepath=None):
    """Load scenario-specific LLM parameter tables.

    Returns a dict with:
      - "access": jnp.array of shape (16, 4)  — per scenario x action
      - "effort": jnp.array of shape (16, 4)  — per scenario x action
      - "reward": jnp.array of shape (16,)    — per scenario (scenario-level)
    ...
    reward = np.zeros((len(SCENARIO_LABELS),), dtype=np.float32)
    for _, row in df.iterrows():
        s_idx = SCENARIO_TO_IDX[row["scenario_label"]]
        a = int(row["action"])
        access[s_idx, a] = row["access"]
        effort[s_idx, a] = row["effort"]
        # Reward is scenario-level; overwritten 4x per scenario with the same value
        reward[s_idx] = row["reward"]
    return {
...
def _require_llm_tables():
    if LLM_TABLES is None:
        raise RuntimeError(
            "LLM_TABLES not loaded. Run `uv run python model/lm_scenario_params.py` "
            "first to generate model/outputs/lm_scenario_params.csv."
        )
...
# ==============================================================================
# LLM-Parameterized Access Models
# ==============================================================================
# These variants replace the fixed vectors access(a), effort(a), and V(a|s)
# with per-scenario values generated by an LLM (Llama-3.3-70B via Together AI;
# see model/lm_scenario_params.py). Scenario index is a new memo dimension.
# The scenario-specific tables are passed as memo parameters rather than
# read from module globals, so model_utils.py can import even before the
# LLM CSV has been generated.
#
# Utility:
#   U_llm(a|s, I, scen)
#     =  w_v * reward[scen] * action_has_reward(a) * motivation(s)
#      + w_r * access[scen, a] * I
#      - w_d * access[scen, a] * (1 - I)
#      - w_e * effort[scen, a]
# where action_has_reward = [0, 1, 1, 1] (action 0 = not eating -> no reward).


@jax.jit
def get_utility_access_full_llm(
    ...
    V = reward_table[scenario_idx] * action_has_reward * motivation
    return alpha * (
        w_v * V
        + w_r * access * intimacy
        - w_d * access * (1 - intimacy)
...
@jax.jit
def get_utility_access_full_llm_disc(
    action, scenario_idx, relationship_condition, reward_condition,
    alpha, w_v, w_r, w_d, w_e,
    access_table, effort_table, reward_table,
):
    intimacy = get_intimacy(relationship_condition)
    return get_utility_access_full_llm(
        action, scenario_idx, intimacy, reward_condition,
        alpha, w_v, w_r, w_d, w_e,
        access_table, effort_table, reward_table,
    )
...
@jax.jit
def get_utility_access_only_llm(
    action, scenario_idx, intimacy, reward_condition,
    alpha, w_r, w_d,
    access_table, effort_table, reward_table,
):
...
@jax.jit
def get_utility_access_only_llm_disc(
    action, scenario_idx, relationship_condition, reward_condition,
    alpha, w_r, w_d,
    access_table, effort_table, reward_table,
):
    intimacy = get_intimacy(relationship_condition)
    return get_utility_access_only_llm(
        action, scenario_idx, intimacy, reward_condition,
        alpha, w_r, w_d,
        access_table, effort_table, reward_table,
    )
...
@jax.jit
def get_utility_no_access_llm_disc(
    action, scenario_idx, relationship_condition, reward_condition,
    alpha, w_v, w_e,
    access_table, effort_table, reward_table,
):
    intimacy = get_intimacy(relationship_condition)
    return get_utility_no_access_llm(
        action, scenario_idx, intimacy, reward_condition,
        alpha, w_v, w_e,
        access_table, effort_table, reward_table,
    )
...
@memo
def actor_forw_access_full_llm[
    action: actions,
...
@memo
def actor_forw_access_only_llm[
    action: actions,
    scenario_idx: Scenarios,
    intimacy: IntimacyLevels,
    reward_condition: RewardConditions,
...
@memo
def actor_discrete_access_full_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    actor: chooses(
        action in actions,
        wpp=exp(
            get_utility_access_full_llm_disc(
                action, scenario_idx, relationship_condition, reward_condition,
                alpha, w_v, w_r, w_d, w_e,
                access_table, effort_table, reward_table,
            )
...
@memo
def actor_discrete_access_only_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    actor: chooses(
        action in actions,
        wpp=exp(
            get_utility_access_only_llm_disc(
                action, scenario_idx, relationship_condition, reward_condition,
                alpha, w_r, w_d,
                access_table, effort_table, reward_table,
            )
...
@memo
def actor_discrete_no_access_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    actor: chooses(
        action in actions,
        wpp=exp(
            get_utility_no_access_llm_disc(
                action, scenario_idx, relationship_condition, reward_condition,
                alpha, w_v, w_e,
                access_table, effort_table, reward_table,
            )
...
@memo
def actor_continuous_access_full_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
...
@memo
def actor_continuous_access_only_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
...
@memo
def actor_continuous_no_access_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
...
@memo
def observer_intimacy_access_full_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
](alpha, w_v, w_r, w_d, w_e, alpha_observer, access_table: ..., effort_table: ..., reward_table: ...):
    cast: [actor, observer]
    ...
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_continuous_access_full_llm[
                action, scenario_idx, relationship, reward_condition
            ](alpha, w_v, w_r, w_d, w_e, access_table, effort_table, reward_table),
        ),
    ]
...
@memo
def observer_intimacy_access_only_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
](alpha, w_r, w_d, alpha_observer, access_table: ..., effort_table: ..., reward_table: ...):
    cast: [actor, observer]
    observer: knows(scenario_idx)
    observer: knows(reward_condition)
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_continuous_access_only_llm[
                action, scenario_idx, relationship, reward_condition
            ](alpha, w_r, w_d, access_table, effort_table, reward_table),
        ),
    ]
...
@memo
def observer_intimacy_no_access_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship: IntimacyLevels,
    reward_condition: RewardConditions,
](alpha, w_v, w_e, alpha_observer, access_table: ..., effort_table: ..., reward_table: ...):
    cast: [actor, observer]
    observer: knows(scenario_idx)
    observer: knows(reward_condition)
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_continuous_no_access_llm[
                action, scenario_idx, relationship, reward_condition
            ](alpha, w_v, w_e, access_table, effort_table, reward_table),
        ),
    ]
...
@memo
def observer_reward_access_full_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_discrete_access_full_llm[
                action, scenario_idx, relationship_condition, reward_condition
            ](alpha, w_v, w_r, w_d, w_e, access_table, effort_table, reward_table),
        ),
    ]
...
@memo
def observer_reward_access_only_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_discrete_access_only_llm[
                action, scenario_idx, relationship_condition, reward_condition
            ](alpha, w_r, w_d, access_table, effort_table, reward_table),
        ),
    ]
...
@memo
def observer_reward_no_access_llm[
    action: actions,
    scenario_idx: Scenarios,
    relationship_condition: RelationshipConditions,
    reward_condition: RewardConditions,
    ...
    observer: thinks[
        ...
        actor : chooses(
            action in actions,
            wpp=actor_discrete_no_access_llm[
                action, scenario_idx, relationship_condition, reward_condition
            ](alpha, w_v, w_e, access_table, effort_table, reward_table),
        ),
    ]
