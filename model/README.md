...
LM elicitation  (model/lm/)
    score_features.py   →  outputs/lm/lm_scenario_params{,_marginal}.csv  (fixed-action risk + effort)
    generate_alternatives.py --study <slug>  →  outputs/lm/lm_alternatives_<slug>.csv          (per-study counterfactuals)
    score_merged.py          --study <slug>  →  outputs/lm/lm_alternatives_{features,g}_<slug>.csv + canonical CSVs
        ↓
