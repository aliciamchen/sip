"""Generate predictions for food_inv_joint_ie.

Reads jointly-fit utility weights + alpha_observer from this experiment's own
fit_results.csv (NOT transferred between studies), runs the joint_ie observer
on a per-scenario grid, and writes preds_<variant>.npy + preds_summary.csv.
"""
...
def main():
    ...
    for variant, (obs_fn, utility_names, uses_v) in VARIANTS.items():
        ...
        result = obs_fn(**kwargs, **joint_ie_table_kwargs(uses_v))
