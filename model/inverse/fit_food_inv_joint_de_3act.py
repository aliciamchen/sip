"""Fit alpha_observer for food_inv_joint_de_3act.

Study 4a — joint over (reward, effort) given intimacy. Actor params frozen from the canonical food forward fit
(food_forw_intimacy_desire). Writes outputs/food_inv_joint_de_3act/fit_results.csv.
"""
...
def main():
    ...
    data, action, scenario_idx, relationship_condition, resp_reward, resp_effort = load_joint_de_3act_data(EXPERIMENT_SLUG)
    ...
    for variant_name, (obs_fn, kw_names, uses_v) in VARIANTS.items():
        ...
        alpha_observer, nll = fit_joint_de_3act_observer(
            observer_fn=obs_fn,
            actor_params=actor_params[variant_name],
            actor_kwarg_names=kw_names,
            action=action, scenario_idx=scenario_idx, relationship_condition=relationship_condition, response_reward=resp_reward, response_effort=resp_effort,
