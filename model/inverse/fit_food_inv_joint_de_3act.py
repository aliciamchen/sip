"""Fit observer + actor utility weights for food_inv_joint_de_3act.
...
(no transfer from the forward fit). Writes outputs/food_inv_joint_de_3act/fit_results.csv.
"""
...
def main():
    ...
    data, action, scenario_idx, relationship_condition, resp_reward, resp_effort = (
        load_joint_de_3act_data(EXPERIMENT_SLUG)
    )
    ...
    for variant_name, (obs_fn, utility_names, uses_v) in VARIANTS.items():
        ...
        params, nll = fit_joint_de_3act_observer_joint(
            observer_fn=obs_fn,
