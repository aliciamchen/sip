"""
Generate effort-inference predictions for the food_inv-effort_intimacy_alt experiment.

Uses frozen actor parameters from food_forw_intimacy_effort + fitted alpha_observer from
inverse_planning_effort_intimacy_fit_results.csv. The observer-table shape is
...
Outputs:
  food_inv-effort_intimacy_alt_preds_full.csv     — one row per (scenario, action,
    intimacy ∈ {0,50,75,100}, effort_condition); column `density` is the
    posterior probability of that effort condition.
  food_inv-effort_intimacy_alt_preds_summary.csv  — one row per (scenario, action,
...
def _table_kwargs():
    ...
    # does not see the effort paragraph. See fit_inverse_planning_intimacy_effort_intimacy.py
    # for the rationale.
...
def generate_effort_intimacy_preds(
...
def main():
    print("=" * 60)
    print("Generating food_inv-effort_intimacy_alt predictions")
    print("=" * 60)
    ...
    print(
        ...
        "inverse_planning_effort_intimacy_fit_results.csv..."
    )
    ...
    for variant_name, (_obs, _kw) in ACCESS_VARIANTS_EFFORT_INFERRED.items():
        if variant_name not in params:
            ...
            continue
        alpha_observer = alpha_obs.get((variant_name, "effort_intimacy"), 1.0)
        print(f"  {variant_name} (alpha_observer={alpha_observer:.3f})...")
        dfs.append(generate_effort_intimacy_preds(
    ...
    full_path = output_dir / "food_inv-effort_intimacy_alt_preds_full.csv"
    summary_path = output_dir / "food_inv-effort_intimacy_alt_preds_summary.csv"
