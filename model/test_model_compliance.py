...
def test_joint_observer_gradients_match_memo_reference():
    ...
    for family, fast_fn, ref_fn in [
        ...
        # provided no evidence in the regime that matters (review 2026-07-29).
...
def test_parse_run_config_args_informative():
    ...
    RunConfig fields (and it is not the preregistered config)."""
    from _helpers import parse_run_config_args
...
def test_build_priors_kwarg_uniform_is_none():
    """Uniform mode (the preregistered default) yields no priors kwarg — the fit
...
def test_build_priors_kwarg_empty_active_raises():
    ...
    `active` empty; build_priors_kwarg must raise (not silently run the preregistered config)."""
    from _priors import build_priors_kwarg
...
# derived from a prototype that still exists at
# model/diagnostics/tier3_surprise.py — which produced every eta value and
# validation result in notes/candidate-sweep-2026-07-28/RESULTS.md. Two
...
#      survives any future deletion of model/diagnostics/;
#   2. against the prototype itself, when it is importable, so the diagnostics
#      and the reported pipeline are demonstrably the same math.
#
