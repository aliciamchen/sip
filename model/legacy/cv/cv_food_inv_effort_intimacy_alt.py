"""
Leave-one-scenario-out CV for the food_inv_effort_intimacy_alt observer.

Mirrors model/cv/cv_food_inv_intimacy_effort_alt.py but flips the inference direction:
...
Outputs:
  - food_inv_effort_intimacy_alt/cv_preds_summary.csv  — one row per
    (scenario, action, intimacy, model); column `p_effort_high` is the
    LOSO-refit observer's P(effort_high).
  - cv_folds.csv  — per-fold α_observer,
    train/test NLL.
"""
...
EXPERIMENT_SLUG = "food_inv_effort_intimacy_alt"
...
def main():
    print("=" * 60)
    print("LOSO cross-validation: food_inv_effort_intimacy_alt")
    print("=" * 60)
    ...
    output_dir = get_project_root() / "model" / "outputs" / "food_inv_effort_intimacy_alt"
