"""
Leave-one-scenario-out CV for the inv_plan_effort observer.
...
Mirrors model/cv/loso_inverse_alt.py but adapted for:
  ...
  - Frozen actor weights from forw_plan_effort (NOT the canonical forw_plan fit).

Per fold: hold out one scenario, refit α_observer on remaining 15, generate
held-out posterior predictions using the refit α.

Outputs:
  - cv_loso_inv_plan_effort_preds_summary.csv  — one row per (scenario, action, effort, model);
    expected_intimacy from LOSO-refit observer.
  - cv_loso_inverse_effort_folds.csv  — per-fold α_observer, train/test NLL.
"""
...
from fit_inverse_planning import _fit_alpha_observer, compute_intimacy_nll
...
def _loso_intimacy_effort(actor_params_by_model):
    ...
    for variant in VARIANTS:
        if variant not in actor_params_by_model:
            ...
            continue
        obs_fn, kw_names, needs_prior = ACCESS_VARIANTS_EFFORT[variant]
...
def main():
    print("=" * 60)
    print("LOSO cross-validation: inv_plan_effort")
    print("=" * 60)
    ...
    preds_path = output_dir / "cv_loso_inv_plan_effort_preds_summary.csv"
    folds_path = output_dir / "cv_loso_inverse_effort_folds.csv"
