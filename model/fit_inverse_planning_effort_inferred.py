"""
Fit observer alpha for the effort-inference experiment (inv_plan_effort_inferred).

Parallel to model/fit_inverse_planning_effort.py, but flips the inference
direction:
...
def load_effort_inferred_data(filepath: str = None):
    """Load and preprocess inv_plan_effort_inferred data (posterior stage only).
    ...
    print("Loading inv_plan_effort_inferred data...")
...
def fit_effort_inferred_observer(
...
def main():
    ...
    for variant_name, (obs_fn, kw_names, needs_prior) in ACCESS_VARIANTS_EFFORT_INFERRED.items():
        ...
        print(f"{'-' * 40}")
        alpha_observer, nll = fit_effort_inferred_observer(
            observer_fn=obs_fn,
    ...
    results_path = output_dir / "inverse_planning_effort_inferred_fit_results.csv"
