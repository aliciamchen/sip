"""
Observer models — the inverse-planning Bayesian inference layer, in plain JAX.
One family per active study, each over the padded LM-alternatives action space:
...
All twelve observers are direct Bayesian inversions of the actor policies
(`actors.py`), computed in log space (`_sharpened_posterior_logspace`): the
observer posterior is a masked softmax of alpha_observer * log(actor policy)
over the latent axes.
...
The memo originals — the executable specification of these semantics — are in
`memo_spec.py`: `_observer_*_memo_reference` for the observers, plus the actor
memos that the plain-JAX policies in `actors.py` mirror. CHANGE MODEL SEMANTICS
IN BOTH, and `test_model_compliance.py` verifies production ≡ spec on every
variant, in the parameter regime where the spec is numerically healthy.

Dependency layer 3: imports from `actors.py` (which imports `tables.py` and
`utility.py`).
"""
...
# reference (`memo_spec._observer_joint_*_memo_reference`), the observer's
# belief about the actor's latent pair after observing the slot is, by Bayes
# with the actor's UNIFORM latent prior,
#
...
#
# The actor policies already produce the full policy table (a few MB), so the
...
# arguments (minus alpha_observer) verbatim to its actor policy, mirroring the
...
# ==============================================================================
# Fast single-latent observers (plain-JAX inversions of the actor policies)
# ==============================================================================
# Same construction as the joint fast paths: the actor policy supplies the slot
...
# `_sharpened_posterior_logspace`). The memo originals are kept in
# `memo_spec.py` as `_observer_*_memo_reference` — the authoritative statement
# of the semantics; change model semantics in both — and
# test_model_compliance.py enforces fast ≡ reference on every variant.
# Converted alongside the log-space sharpening: the powering the memo
# observers apply inside `wpp` (E[latent == z] ** alpha_observer) has the same
# float32 underflow as the old joint path and cannot be stabilized from
