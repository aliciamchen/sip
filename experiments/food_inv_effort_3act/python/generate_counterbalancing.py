...
ALL_CELLS = [(r, i, a) for r in ("low", "high") for i in (0, 50, 75, 100) for a in ("action_0", "action_1", "action_2")]


def cell_to_dict(cell):
    reward, intimacy, action = cell
    return {"reward_condition": reward, "intimacy_condition": intimacy, "action_condition": action}
...
output_path = (
    get_project_root() / "experiments" / "food_inv_effort_3act" / "json" / "full_counterbalancing.json"
)
