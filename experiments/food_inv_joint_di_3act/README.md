...
**Inferred by participant**: reward_condition AND intimacy (two sliders per phase)

## Trial structure

Each trial shows: vignette + effort paragraph + observed action (at posterior). No intimacy preamble since intimacy is one of the inferred variables. Reward paragraph is hidden.

Slider responses: two sliders per phase. Desire slider with reward_low/high paragraph endpoints; intimacy slider on the 0–100 numeric scale (maximally formal → maximally intimate)..

Each participant sees 16 trials (one per scenario) with cells balanced across participants. Cell space: 6 cells. The counterbalancing script (`python/generate_counterbalancing.py`) produces 192 sequences (12 rounds × 16 rotations), each a 16-trial assignment of factor cells to the 16 scenarios.
...
```bash
bin/deploy-experiment food_inv_joint_di_3act
```
...
https://web.mit.edu/aliciach/www/sip/experiments/food_inv_joint_di_3act/
