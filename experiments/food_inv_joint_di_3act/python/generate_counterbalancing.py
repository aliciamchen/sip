"""Counterbalancing for food_inv_joint_di_3act.

Study 4b — Joint inference over desire + intimacy (known effort).
Cells: effort × action = 2 × 3 = 6 cells.
...
ALL_CELLS = [(e, a) for e in ("low", "high") for a in ("action_0", "action_1", "action_2")]


def cell_to_dict(cell):
    effort, action = cell
    return {"effort_condition": effort, "action_condition": action}
...
output_path = (
    get_project_root() / "experiments" / "food_inv_joint_di_3act" / "json" / "full_counterbalancing.json"
)
...
print(f"Cells: {len(ALL_CELLS)} (effort × action = 2 × 3 = 6 cells)")
