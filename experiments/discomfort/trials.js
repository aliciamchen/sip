    ...
    pages: [
      `
            <div class="instructions-container">
                ...
                <p>For each action, we will ask you to evaluate how uncomfortable the two people would be with sharing food in that way. Please consider each option independently.</p>
  ...
  stimuli.forEach((stimulus, stimulusIndex) => {
    ...
    if (stimulusIndex === 14) {
      ...
      const attentionCheckPrompts = [
        `Please select "Not at all uncomfortable."`,
        `Please select "Extremely uncomfortable."`,
        `Please select "Moderately uncomfortable."`,
        `Please select "Somewhat uncomfortable."`,
      ];
      for (let i = 0; i < 4; i++) {
        attentionCheckQuestions.push({
          prompt: `<div class="action-text">${attentionCheckPrompts[i]}</div>`,
          name: `attention_check_${i}`,
          labels: discomfort_labels,
          required: true,
        });
      }
    ...
    trials.push({
      ...
      stimulus: `
        ...
        stimuli.length
                        ...
                        <p class="closeness-info">Consider ${stimulus.name_0} and ${stimulus.name_1}, who would describe their relationship as <strong>${closeness_texts[stimulus.closeness_condition]}</strong>.</p>
    ...
    trials.push({
      ...
      preamble: `
        <div>
          <p class="closeness-info">Consider ${stimulus.name_0} and ${stimulus.name_1}, who would describe their relationship as <strong>${closeness_texts[stimulus.closeness_condition]}</strong>.</p>
          <p>${stimulus.vignette}</p>
          <p><strong>For each action, please rate how uncomfortable the two people would be with sharing food in that way.</strong></p>
        </div>
      `,
      randomize_question_order: false,
      button_label: "Continue",
