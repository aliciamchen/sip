"""Counterbalancing for food_inv_joint_de_3act.

Study 4a — Joint inference over desire + effort (known intimacy).
Cells: intimacy × action = 4 × 3 = 12 cells.
...
def cell_to_dict(cell):
    intimacy, action = cell
    return {"intimacy_condition": intimacy, "action_condition": action}
...
output_path = (
    get_project_root() / "experiments" / "food_inv_joint_de_3act" / "json" / "full_counterbalancing.json"
)
...
print(f"Cells: {len(ALL_CELLS)} (intimacy × action = 4 × 3 = 12 cells)")
