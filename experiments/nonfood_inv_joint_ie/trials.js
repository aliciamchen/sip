// Study 3b — Joint inference over intimacy and effort, given desire, on the
// nonfood scenario set (scenarios_nonfood.csv). Same design as Study 2b
// (food_inv_joint_ie): 2 (desire) × 3 (observed action). Desire paragraph shown
// after the vignette; both sliders on one page per phase — an intimacy 0-100
// slider and an effort slider between the two effort paragraphs. No preamble
// page.
...
export const CONFIG = makeConfig("nonfood_inv_joint_ie");
export const INSTRUCTIONS_PAGES = STUDY_INSTRUCTIONS.nonfood_inv_joint_ie;
export const COMPREHENSION_QUESTIONS =
  STUDY_COMPREHENSION_CHECKS.nonfood_inv_joint_ie;
...
export function makeStimulusTrials(jsPsych, stimuli) {
  ...
  stimuli.forEach((stimulus, index) => {
    ...
    const memoryCheck = makeMemoryCheckForStimulus(
      stimulus,
      NONFOOD_MEMORY_CHECKS,
    );
