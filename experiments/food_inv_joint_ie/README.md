...
Each participant sees 16 trials (one per scenario) with cells balanced across participants. Cell space: 6 cells. The shared counterbalancing generator (`_lib/python/counterbalancing.py`) produces 96 sequences (6 rounds × 16 rotations), each a 16-trial assignment of factor cells to the 16 scenarios.
