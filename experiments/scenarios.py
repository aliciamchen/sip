...
sharing) plus separable `reward_low`/`reward_high` and `effort_low`/`effort_high`
...
- `action_0` = the no-share action.
- `action_1` = the low-risk (non-saliva) sharing action, written so the effort cost
  is carried by the `effort_low` / `effort_high` paragraph rather than baked into the
  action text.
- `action_2` = the high-risk sharing action.
