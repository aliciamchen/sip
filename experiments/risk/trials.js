    ...
    pages: [
      `
            <div class="instructions-container">
                ...
                <p>For each action, we will ask you to evaluate how much saliva is likely to be exchanged between the two people, in the context of the scenario. Please consider each option independently.</p>
  ...
  stimuli.forEach((stimulus, stimulusIndex) => {
    ...
    if (stimulusIndex === 14) {
      ...
      const attentionCheckPrompts = [
        `Please select "No saliva at all."`,
        `Please select "Very much saliva."`,
        `Please select "Moderate saliva."`,
        `Please select "Little saliva."`,
      ];
      for (let i = 0; i < 4; i++) {
        attentionCheckQuestions.push({
          prompt: `<div class="action-text">${attentionCheckPrompts[i]}</div>`,
          name: `attention_check_${i}`,
          labels: saliva_labels,
          required: true,
        });
      }
    ...
    trials.push({
      ...
      preamble: `
        <div>
          <p>${stimulus.vignette}</p>
          <p><strong>For each action, please rate how much saliva from one person is likely to end up in the other person's mouth.</strong></p>
        </div>
      `,
      randomize_question_order: false,
      button_label: "Continue",
