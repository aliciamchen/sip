"""Counterbalancing for food_inv_desire_3act.
...
ALL_CELLS = [(e, i, a) for e in ("low", "high") for i in (0, 50, 75, 100) for a in ("action_0", "action_1", "action_2")]


def cell_to_dict(cell):
    effort, intimacy, action = cell
    return {"effort_condition": effort, "intimacy_condition": intimacy, "action_condition": action}
...
output_path = (
    get_project_root() / "experiments" / "food_inv_desire_3act" / "json" / "full_counterbalancing.json"
)
...
print(f"Cells: {len(ALL_CELLS)} (effort × intimacy × action = 2 × 4 × 3 = 24 cells)")
