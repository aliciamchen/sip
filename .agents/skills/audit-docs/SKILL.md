---
name: audit-docs
description: Audit READMEs, AGENTS.md, and rules files for staleness, redundancy, broken links, and structural issues. Reports concrete fixes (with line numbers) separately from bigger structural suggestions the user can decide on.
...
- `.Codex/AGENTS.md` (Codex-only project context)
- `.Codex/rules/{analysis,data,experiments,model}.md` (path-conditional rules)
...
3. **Audience fit.** Each doc should serve one audience clearly:
   ...
   - `AGENTS.md` → Codex (terminology drift, naming conventions, gitignored Overleaf folders, anything not in the code or public docs)
   - rules files → Codex when working in a specific dir (directory-specific concerns, not general project context)
