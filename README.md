...
- `V`: signed valence per (scenario, action, motivation) on a -3 to +3 scale, normalized to [-1, +1]. -3 = action is strongly counterproductive for the actor's current state (e.g., eating heartily when full); 0 = neutral; +3 = action strongly serves the state. Produced by `model/lm_scenario_params.py --feature v` → `model/outputs/lm_scenario_v.csv`.
- `access`: how much each action opens each person up to the other — physically (bodily substance transfer, skin contact, spatial proximity), informationally, or both, on a 0-6 scale normalized to [0, 2]. Produced by `model/lm_scenario_params.py` → `model/outputs/lm_scenario_params.csv`.
...
- `food_inv-intimacy_desire_noalt`: observer sees motivation, infers intimacy. Alternatives are conditioned on (scenario, observed action, motivation). Generated by `model/lm_generate_alternatives.py` (default `--conditioning motivation`) → `model/outputs/lm_alternatives.csv`. Features (access, effort) are scored by `model/lm_scenario_params.py --score-alternatives` → `lm_alternatives_features.csv`. V is scored under both motivation states by `model/lm_scenario_params.py --feature v_alternatives` → `lm_alternatives_v.csv`.
- `food_inv-desire_intimacy_noalt`: observer sees relationship, infers motivation. Alternatives are conditioned on (scenario, observed action, relationship). Generated by `model/lm_generate_alternatives.py --conditioning relationship` → `lm_alternatives_relationship.csv` (256 cells: 16 × 4 × 4). Features by `--feature access_effort_alternatives_relationship` → `lm_alternatives_relationship_features.csv`; V (per motivation_query) by `--feature v_alternatives_relationship` → `lm_alternatives_relationship_v.csv`.
...
uv run python model/lm_scenario_params.py --feature v_alternatives        # V for LM-generated alternatives (food, required for no-alt observers)
```
