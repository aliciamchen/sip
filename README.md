...
The paper's figures are generated by the Python plotting scripts, all styled through the shared `plot_style.py` (the single source of truth for palettes and fonts): the main results figures come from `figures/results/` (`make figures-results`) and the SI LM-validation figures from `model/lm/` (`make figures-lm-si`). The analysis qmds report demographics and data checks rather than producing any figure.
