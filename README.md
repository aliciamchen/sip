...
figures/           Paper figures
  ...
  paper_panels/    Illustrator-linked results panels, one per sub-study, plus the shared legends (PDF)
...
The per-study results figures are assembled by hand in Illustrator, so their pieces are generated separately by `make figures-panels`, which writes one four-column panel row per sub-study plus the four legends they share into `figures/paper_panels/` as PDFs whose text stays editable in Illustrator. The points design those panels use is shared with the assembled preview figures in `figures/outputs/` (`study1a_results.pdf` and friends), so the two cannot disagree.
