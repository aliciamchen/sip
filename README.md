...
The no-alt observer experiments (`inv_plan_intimacy_noalt`, `inv_plan_desire_noalt`) need V scored not just for the four canonical actions but also for the LM-generated counterfactual alternatives. That's a separate `model/lm_scenario_params.py --feature v_alternatives` pass that writes `model/outputs/lm_alternatives_v.csv`.
