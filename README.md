...
- `food_inv-intimacy_desire_noalt`: observer sees motivation, infers intimacy. Alternatives are conditioned on (scenario, observed action, motivation). Generated by `model/lm/generate_alternatives_motivation.py` (default `--conditioning motivation`) → `model/outputs/lm/lm_alternatives_food_inv-intimacy_desire_noalt.csv`. Features (access, effort) are scored by `model/lm/score_alternative_features.py` → `lm_alternatives_features_food_inv-intimacy_desire_noalt.csv`. V is scored under both motivation states by `model/lm/score_alternative_v.py` → `lm_alternatives_v_food_inv-intimacy_desire_noalt.csv`.
- `food_inv-desire_intimacy_noalt`: observer sees relationship, infers motivation. Alternatives are conditioned on (scenario, observed action, relationship). Generated by `model/lm/generate_alternatives_relationship.py` → `lm_alternatives_food_inv-desire_intimacy_noalt.csv` (256 cells: 16 × 4 × 4). Features by `--feature access_effort_alternatives_relationship` → `lm_alternatives_features_food_inv-desire_intimacy_noalt.csv`; V (per motivation_query) by `--feature v_alternatives_relationship` → `lm_alternatives_v_food_inv-desire_intimacy_noalt.csv`.
