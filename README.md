...
That primary metric is a global fit index, which is a low-powered test of the specific claim these studies make. The paper's hypothesis is about a modulation -- how the condition that is *given* to the observer changes what an action reveals about the variables that are not -- and that modulation accounts for only 1-3% of the variance in individual trials, most of which is response noise no model can predict. Two further sets of statistics, computed by `model/cv/contrast_tests.py` and written into the same JSON file, are reported alongside it: a variance decomposition that measures where a study's trial-level variance actually lives, and a gradient test that scores the modulation itself, comparing how much each model variant's held-out predictions change across the given condition's levels against how much human judgments do. Both are secondary to the preregistered comparison and are reported beside it rather than in place of it.
...
`model/cv/generalization_primary.py` (`make generalization-primary`) therefore scores the same arms on the two measures that are sensitive to it — the condition-averaged model-vs-human correlation the paper reports, and the fraction of participants' modulation each arm recovers. Nothing is refitted, since the transfer and pooled runs already wrote standard cross-validation output sets; this reads their held-out predictions, so it is cheap and needs re-running whenever either of them is. The three measures do not always rank the arms the same way, which is the expected consequence of a modulation worth a few percent of trial variance, and the paper reports all three.
