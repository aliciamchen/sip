...
- **Inverse planning, desire inference, no alternatives shown** (`inv_plan_desire_noalt/`) — same noalt setup applied to the desire-inference direction; observers see the chosen action and a desire slider whose endpoints are the scenario's `reward_low` / `reward_high` paragraphs
...
The no-alt observer experiments (`inv_plan_intimacy_noalt`, `inv_plan_desire_noalt`) need V scored not just for the four canonical actions but also for the LM-generated counterfactual alternatives. The two experiments use different conditioning axes for those alternatives, following the design rule that alternatives should be conditioned on what the observer can see (not on the latent the observer infers):

- `inv_plan_intimacy_noalt`: observer sees motivation, infers intimacy. Alternatives are conditioned on (scenario, observed action, motivation). Generated by `model/lm_generate_alternatives.py` (default `--conditioning motivation`) → `model/outputs/lm_alternatives.csv`. Features (access, effort) are scored by `model/lm_scenario_params.py --score-alternatives` → `lm_alternatives_features.csv`. V is scored under both motivation states by `model/lm_scenario_params.py --feature v_alternatives` → `lm_alternatives_v.csv`.
- `inv_plan_desire_noalt`: observer sees relationship, infers motivation. Alternatives are conditioned on (scenario, observed action, relationship). Generated by `model/lm_generate_alternatives.py --conditioning relationship` → `lm_alternatives_relationship.csv` (256 cells: 16 × 4 × 4). Features by `--feature access_effort_alternatives_relationship` → `lm_alternatives_relationship_features.csv`; V (per motivation_query) by `--feature v_alternatives_relationship` → `lm_alternatives_relationship_v.csv`.
...
uv run python model/cv/loso_inverse_effort.py                # Effort inverse planning, intimacy inference
uv run python model/cv/loso_inverse_effort_inferred.py       # Effort inverse planning, effort inference
```
