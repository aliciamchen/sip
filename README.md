...
- `V`: signed valence per (scenario, action, motivation) on a -3 to +3 scale, normalized to [-1, +1]. -3 = action is strongly counterproductive for the actor's current state (e.g., eating heartily when full); 0 = neutral; +3 = action strongly serves the state. Produced by `model/lm/scenario_params.py --feature v` → `model/outputs/lm_scenario_v.csv`.
- `access`: how much each action opens each person up to the other — physically (bodily substance transfer, skin contact, spatial proximity), informationally, or both, on a 0-6 scale normalized to [0, 2]. Produced by `model/lm/scenario_params.py` → `model/outputs/lm_scenario_params.csv`.
...
- `food_inv-intimacy_desire_noalt`: observer sees motivation, infers intimacy. Alternatives are conditioned on (scenario, observed action, motivation). Generated by `model/lm/generate_alternatives.py` (default `--conditioning motivation`) → `model/outputs/lm_alternatives.csv`. Features (access, effort) are scored by `model/lm/scenario_params.py --score-alternatives` → `lm_alternatives_features.csv`. V is scored under both motivation states by `model/lm/scenario_params.py --feature v_alternatives` → `lm_alternatives_v.csv`.
