...
figures/           Paper figures
  ...
  paper_panels/    Illustrator-linked results panels, one per sub-study, plus the shared legends (SVG + PDF)
...
The fits and cross-validation default to the preregistered configuration — uniform priors over the inferred latent variables — and this default reproduces the paper's numbers, writing its outputs to `model/outputs/<slug>/` byte-for-byte as before. So that revisions of the model can be evaluated without disturbing that registered baseline, both `make fit-<slug>` and `make cv-<slug>` accept a run-configuration variable that switches the observer onto informative LM-elicited priors (`PRIORS=informative`, or `PRIORS=informative:<latents>` for a subset). Any informative configuration writes its outputs to a separate `model/outputs/<slug>/alt/<tag>/` directory whose tag encodes the configuration, so the preregistered baseline and each experimental variant coexist on disk and can be compared on matched trials. This keeps exploratory model development running alongside the registered analysis rather than overwriting it.
...
The per-study results figures are assembled by hand in Illustrator, so their pieces are generated separately by `make figures-panels`, which writes one four-column panel row per sub-study plus the four legends they share into `figures/paper_panels/`, as an SVG and PDF pair whose text stays editable. The points design those panels use is shared with the assembled preview figures in `figures/outputs/` (`study1a_results.pdf` and friends), so the two cannot disagree.

Because the main results figures average over the 16 scenarios, `make figures-si-scenarios` renders the supplementary per-scenario view: one 4x4 facet grid per study (`si_scenarios_study1a.pdf` and friends), where each facet is a single scenario's human cell means with bootstrap CIs and the full model's out-of-sample prediction drawn as a short dash at the same position.
