...
The R/Quarto qmds in `analysis/` report demographics and data checks only — every figure comes from the Python scripts in `figures/scripts/` (the main results figures, run with `make figures-results`; the SI LM figures, run with `make figures-lm-si`) and the model-comparison statistics from `model/cv/model_comparison.py` (see `.Codex/rules/analysis.md`).
