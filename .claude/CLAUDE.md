...
## Legacy is archived, not maintained

Everything under `legacy/` (the `data/legacy/` experiments, `experiments/legacy/` scenario sets, `analysis/legacy/` qmds, the `LEGACY_FORWARD`/`LEGACY_INVERSE` roster, the `_alt`/`_noalt` inverse slugs, and the old pilot data) is kept for reference only — it is **not maintained**. Don't spend effort keeping legacy working or consistent with the active roster. When a change to the active pipeline (a shared `_lib/` helper, a scenario column, a converter, a model loader, a renamed file) would otherwise break legacy regeneration, fits, or analysis, just let it break — don't refactor legacy to keep it running, and don't bother preserving byte-identical legacy outputs. Prioritize the four active inverse studies; treat anything under `legacy/` as frozen and disposable (leave it in place — archived, not deleted — but don't invest in it).
