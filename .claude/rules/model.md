...
A **run config** selects which prior the observer uses, defaulting to the preregistered pipeline (uniform priors over the latent grids, outputs under `outputs/<slug>/`). Both `fit_<slug>.py` and the CV dispatcher parse it from two shared flags (`_helpers.parse_run_config_args` → `RunConfig` in `model/run_config.py`):
