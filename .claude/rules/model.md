...
|---|---|---|---|---|---|
| `food_inv_desire`   | 1a | `observer_reward_*`    | `actor_discrete_3act_*_padded_desire`     | desire (reward)        | (padded_slot, scenario, observed_action, effort, relationship, reward) |
| `food_inv_joint_de` | 1b | `observer_joint_de_*`  | `actor_discrete_3act_*_padded_joint_de`   | desire + effort        | (padded_slot, scenario, observed_action, relationship, reward, effort) |
...
- `tables.py` — enums (`Scenarios`, `RewardConditions`, `RelationshipConditions`, `EffortConditions`, `IntimacyLevels`, `PaddedActionSlots`, `PaddedActionSlots3Act`), action arrays (`actions`, `actions_effort`, `actions_3act`), `SCENARIO_LABELS`, the fixed-action 3-action loaders (`LLM_TABLES_3ACT`, `load_lm_v_3act`), the per-study padded loaders (`load_padded_lm_tables_3act_{desire,joint_de,intimacy,joint_ie}`), and the legacy 4-action / 2-action loaders (`LLM_TABLES`, `LLM_TABLES_EFFORT`, `load_lm_v`).
- `utility.py` — jit-compiled utility functions. Active: the padded families `get_utility_3act_{full,discomfort_only,base}_padded_{desire,joint_de,intimacy,joint_ie}` plus their `get_prior_padded_3act_*` and `get_lm_v_padded_3act_*` helpers. The fixed-action `get_utility_3act_*` (and `_disc` siblings) and the legacy canonical/effort families remain.
...
- `observers.py` — observer memos, one family per active study (`observer_reward_*`, `observer_joint_de_*`, `observer_intimacy_*`, `observer_joint_ie_*`), each in `_full` / `_discomfort_only` / `_base`. The legacy padded `_noalt` observers also live here.
...
- `score_3act_v.py` — fixed-action signed-valence V (16 × 3 × 2). → `outputs/lm/lm_scenario_v_3act.csv`.
...
- `score_3act_merged.py --study <slug>` — scores the unified [canonical + unique alts] list per scenario on access (effort-marginal), effort (per effort_condition), and V (per motivation), so slot 0 and slots 1..k share one comparative frame. Writes the shared canonical CSVs + per-study `lm_alternatives_features_<slug>.csv` and `lm_alternatives_v_<slug>.csv`. For studies whose observer **infers** effort (1b, 2b), each alt's effort feature is emitted for both effort conditions (effort is a feature axis, not a generation axis).
- `_features_dispatcher.py` — internal multi-mode helper for the canonical scorers.

**Three design choices in merged scoring:** (1) canonical + alts scored together (shared comparative frame); (2) access is effort-marginal — access(a|s) is formally intimacy- and effort-independent (modulated by `(1-I)^γ` in the utility), so it's elicited without the effort paragraph and broadcast; (3) V is LM-elicited per motivation, not derived from `is_share` (`is_share` is preserved only as diagnostic metadata). Note: the journal manuscript at `SIP_journal/main.tex` still describes V as derived from `is_share` — that's out of date and should be updated to describe LM-elicited V.
