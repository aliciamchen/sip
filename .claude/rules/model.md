...
- `run_delta_io.py` — the **reader** for those per-run deltas, shared by the SI run-spread figure, `export_results_latex.py` (the `\runSpread*` macros), and `run_deltas.py` itself. It tries `cv_preds_summary.json` first and falls back to the sidecar, refusing a sidecar whose recorded `cv_preds_summary.json` hash no longer matches (a stale sidecar's spread read against a fresh σ would be a mixed-vintage number). Raises `RunDeltasUnavailable` rather than returning None, so each caller picks its own policy: the figure skips the study, the exporter lets it propagate. Stdlib + numpy only, like `_checkpoint.py`, so a figure script can import it without a JAX import.
