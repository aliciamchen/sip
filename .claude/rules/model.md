...
- `actors.py` — actor memos: `actor_forw_*` (forward, canonical), `actor_forw_effort_*` (forward, effort experiment), `actor_forw_3act_*` (forward 3-action), `actor_continuous_3act_*` and `actor_discrete_3act_*` (3-action inverse actors used inside Studies 2/3/4 observer `thinks[...]` blocks). The 4-action `actor_continuous_*_padded` and `actor_continuous_*_padded_rel` variants remain for the legacy `_noalt` inverse experiments.
...
Preregistration documents are in `preregs/` at the repo root (one file per active experiment slug). Sandboxed/experimental code is in `model/sandbox/`.
