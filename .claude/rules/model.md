...
Fixed vectors: `access(a) = [0, 0.3, 1, 2]` (graded exposure; saliva transfer is a qualitatively bigger step than just eating together), `effort(a) = [0, 1, 1, 1]`. `V(a|s)` reuses `get_reward_base` from the pre-registered family (0 for action 0 or LOW motivation; 1 otherwise).
