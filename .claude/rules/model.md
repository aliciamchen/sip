...
### Informative-prior / alternatives-vintage configs

A **run config** selects which prior the observer uses and which alternatives vintage the LM tables come from, defaulting to the preregistered canonical pipeline (uniform priors over the latent grids, the unsuffixed `lm_runs.jsonl`, outputs under `outputs/<slug>/`). Both `fit_<slug>.py` and the CV dispatcher parse it from three shared flags (`_helpers.parse_run_config_args` → `RunConfig` in `model/run_config.py`), threaded through the table-kwargs builders and `_FAMILIES` (spec: `notes/2026-07-18-informative-priors-refusal-alts-design.md`):
...
- `--alts-suffix "" | _refusal_hint | _refusal_hint_hyp` — the alternatives vintage; routes the loaders to `lm_runs<suffix>.jsonl` (and its `_base` counterpart).
- `--priors-file <name>` — override the priors JSONL name (default `lm_priors.jsonl`); used by the human-ceiling check below.

The Makefile passes these through on `fit-<slug>` / `cv-<slug>` via the `PRIORS`, `ALTS_SUFFIX`, and `PRIORS_FILE` variables (assembled into `CONFIG_FLAGS`), e.g. `make cv-food_inv_joint_de PRIORS=informative ALTS_SUFFIX=_refusal_hint`.
...
**Output layout.** The canonical config (uniform × unsuffixed) keeps writing `outputs/<slug>/`, byte-identical to the pre-config pipeline. Every non-canonical config writes the same file set + manifests under `outputs/<slug>/alt/<tag>/`, where the tag encodes the config (`RunConfig.tag()`: e.g. `informative_current`, `uniform_refusal_hint`, `informative-desire_current`). The CV checkpoint fingerprint hashes `lm_priors*.jsonl` and the flag values alongside the data / tables / warm-start / model-code, so config vintages can never splice.
...
**Alternatives arms and the smoke gate.** `generate_alternatives.py --arm refusal_hint|refusal_hint_hyp` writes the suffixed alternatives vintages (`refusal_hint` adds a refusal-friendly clause to the generation prompt; `refusal_hint_hyp` additionally conditions generation on the participant's epistemic state about the inferred latents), rescored by `score_merged.py --alts-suffix <suffix>`. Before any paid K=20 run, `make smoke-alternatives-<slug>` echoes the K=1 three-arm commands (current baseline + the two arms) followed by `model/lm/smoke_report.py --study <slug>`, which reports refusal-alternative coverage, set-size distribution, empty-set rate, and prompt-contamination metrics so the arm can be chosen deliberately.

**Config comparison.** `model_comparison.py --compare-configs <a> <b>` scores two config dirs (`canonical`, or a tag) on the trials they share, matching on (subject_id, scenario_label) and reporting the mean per-trial held-out LL difference with the standard participant bootstrap; it verifies both manifests against the same data CSV. This is the attribution-grid tool (forward and backward marginals over the {priors} × {alts} lattice) and writes `outputs/<slug>/alt/compare_<a>_vs_<b>.json`.
