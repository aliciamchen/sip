...
3. **Output CSV consistency.** Sample headers from `model/outputs/<slug>/fit_results.csv` and `cv_folds.csv` across experiments. The `experiment` column should always hold the slug. Every fit carries all utility weights plus `alpha_observer` (columns `param_w_v`, `param_w_d`, `param_w_e`, `param_gamma` — the reward-term weight is intentionally named `w_v`, not a typo for `w_d`). Also scan fitted values for parameters pinned at the 1e-6 lower bound and report them: a boundary fit means that term has collapsed out of the model.
